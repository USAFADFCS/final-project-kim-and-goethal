    ...
    def use(self, ip: str) -> str:
        try:
            ...
            for v in summary["vulnerabilities"]:
                ...
                output_str = v['output'][:500]  # Extended length
...
async def main():
    ...
    # Planner with shortened role definition for smaller model
    planner = SimpleReActPlanner(llm, tool_registry)
    planner.prompt_builder.role_definition = RoleDefinition(
        "You are a cybersecurity analyst for educational demos. Follow this workflow EXACTLY: "
        "1. Extract IP. "
        "2. Use run_nmap, analyze for OS/ports/vulns/CVEs. "
        "3. For key CVEs, use query_cve. Pick relevant one (e.g., file upload). "
        "4. Use query_kb for payloads (reverse shells and PHP webshell). "
        "5. Use get_attacker_ip. "
        "6. Use generate_payload type='webshell'. "
        "7. Use deploy_payload on HTTP port. "
        "8. Use curl_page to parse upload response for path (look for 'access it via:'). "
        "9. Use generate_payload type='reverse' with OS/IP/port. "
        "10. Use start_listener on port. "
        "11. Use execute_cmd with path and reverse command. "
        "If fails, try try_multiple_payloads. If all fails, explain and ask user. "
        "Status after each tool. Format responses as: Thought: [text] Action: [tool] with args. Or Final Answer: [text]."
    )
