...
but 
Modification: Switched from Hugging Face LLM to OpenAI API using OPENAI_API_KEY for cloud-based generation.
"""
...
# Suppress warnings (removed HF-specific ones)
...
# Custom OpenAI Adapter to mimic fairlib's LLM interface (uses ainvoke with messages)
class OpenAIAdapter:
    ...
    async def ainvoke(self, messages, **kwargs):
        # Convert fairlib Message objects to plain dicts for OpenAI
        serialized_messages = []
        for msg in messages:
            if isinstance(msg, Message):
                # Extract role and content (add more attrs if needed, e.g., tool_calls)
                serialized_msg = {
                    "role": msg.role,
                    "content": msg.content
                }
                ...
                serialized_messages.append(msg)  # Already a dict? Pass through

        response = await self.client.chat.completions.create(
            model=self.model,
            messages=serialized_messages,
            **kwargs  # e.g., temperature=0.7, max_tokens=512
        )
        # Wrap the OpenAI response string in a fairlib Message object
        return Message(content=response.choices[0].message.content, role="assistant")
...
async def main():
    ...
    # Load LLM using OpenAI Adapter
    llm = OpenAIAdapter(model_name="gpt-3.5-turbo", api_key=api_key)
