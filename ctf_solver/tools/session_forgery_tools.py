...
class FlaskSessionForgeryTool:
    ...
    # Common Werkzeug / Flask debug endpoints. These can expose the
    # secret key (via /?__debugger__=yes prompted PIN calculation) or
    # allow direct Python exec through the browser console.
    ...
    # Common Flask secret keys found in CTFs. Combines the original
    # 62-entry CTF-flavored list with 200 entries from flask-unsign's
    # bundled wordlist and ~150 framework defaults / developer habits.
    # Gap E1 (2026-04-17): expanded to ~400 entries after MetaCTF run
    # #14 Microdosing could not crack a Flask cookie with the old set.
    COMMON_SECRETS: List[str] = list(
        dict.fromkeys(
            [
                # --- Original CTF-flavored entries ---
                "secret",
                "secret_key",
                "secretkey",
                "SECRET_KEY",
                ...
                # --- flask-unsign top-200 entries (verbatim, deduped) ---
                ...
                "itsdangerous",
                "dangerous",
                "this-is-not-a-good-secret",
                "debug-secret",
                "debug_secret",
                "super-duper-secret",
                "super_duper_secret",
                "this_is_the_secret_key",
                "ReallyLongAndSuperSecretKey",
                "NyuKuW/b27ZWhXeM",
                ...
                "rails-secret",
                "rails_secret",
                # --- Framework / CTF challenge-name flavored ---
                "picoctf",
                "picoCTF{",
                "metactf",
                "MetaCTF",
    ...
    def use(self, tool_input: str) -> str:
        ...
        elif operation == "debug_endpoint_probe":
            return self._debug_endpoint_probe(data)

        return "[FlaskSessionForgeryTool] Error: Unexpected state."

    def _debug_endpoint_probe(self, data: dict) -> str:
        """Probe common Werkzeug/Flask debug endpoints for accidentally
        exposed consoles. Single HEAD per path, report any non-404
        response with a body snippet.

        Added as Gap E2 (2026-04-17) — MetaCTF run #14 Microdosing ran
        out of steps without ever testing for an exposed /console."""
        url = (data.get("url") or "").strip()
        if not url:
            return (
                "[FlaskSessionForgeryTool] Error: 'url' is required for "
                "debug_endpoint_probe."
            )
        base = url.rstrip("/")
        timeout = int(data.get("timeout", 5))
        lines = [
            "[FlaskSessionForgeryTool] Werkzeug/Flask Debug Endpoint Probe",
            "=" * 60,
            f"Target: {base}",
            "",
        ]
        ...
        for path in self._DEBUG_PATHS:
            ...
            try:
                ...
                continue
            if status == 404:
                lines.append(f"[-] {path}: 404")
                continue
            opened.append((path, status, body))
            lines.append(f"[+] {path}: {status} — {body.strip()[:100]}".rstrip())

        lines.append("")
        if opened:
            lines.append(f"=== {len(opened)} candidate endpoint(s) found ===")
            lines.append(
                "Next step: GET each endpoint fully. /console = Werkzeug "
                "interactive debugger (may require a PIN). With access "
                "to the console you can print app.config['SECRET_KEY'] "
                "or execute arbitrary Python."
            )
        else:
            lines.append("no debug endpoints exposed (all 404)")
        return "\n".join(lines)
