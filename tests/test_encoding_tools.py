...
class TestEncodingTool:
    ...
    def test_base64_encode(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "Hello, World!", "operation": "base64_encode"})
        )
    ...
    def test_base64_decode(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "SGVsbG8sIFdvcmxkIQ==", "operation": "base64_decode"})
        )
    ...
    def test_base64_decode_no_padding(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "SGVsbG8", "operation": "base64_decode"})
        )
    ...
    def test_base64_decode_binary(self):
        ...
        result = self.tool.use(
            json.dumps({"text": binary_b64, "operation": "base64_decode"})
        )
    ...
    def test_url_encode(self):
        """Test URL encoding."""
        result = self.tool.use(
            json.dumps({"text": "Hello World!", "operation": "url_encode"})
        )
    ...
    def test_url_decode(self):
        """Test URL decoding."""
        result = self.tool.use(
            json.dumps({"text": "Hello%20World%21", "operation": "url_decode"})
        )
        assert "Hello World!" in result
    ...
    def test_url_encode_special_chars(self):
        ...
        result = self.tool.use(
            json.dumps(
                {"text": "<script>alert('xss')</script>", "operation": "url_encode"}
            )
        )
        assert "%3C" in result  # <
        assert "%3E" in result  # >
    ...
    def test_hex_encode(self):
        """Test hex encoding."""
        result = self.tool.use(json.dumps({"text": "Hello", "operation": "hex_encode"}))
        assert "48656c6c6f" in result

    def test_hex_decode(self):
        """Test hex decoding."""
        result = self.tool.use(
            json.dumps({"text": "48656c6c6f", "operation": "hex_decode"})
        )
        assert "Hello" in result
    ...
    def test_hex_decode_with_prefix(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "0x48656c6c6f", "operation": "hex_decode"})
        )
        assert "Hello" in result
    ...
    def test_hex_decode_with_spaces(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "48 65 6c 6c 6f", "operation": "hex_decode"})
        )
        assert "Hello" in result
    ...
    def test_rot13(self):
        """Test ROT13 encoding."""
        result = self.tool.use(json.dumps({"text": "Hello", "operation": "rot13"}))
        assert "Uryyb" in result

    def test_rot13_roundtrip(self):
        """Test ROT13 is its own inverse."""
        original = "The quick brown fox"
        result1 = self.tool.use(json.dumps({"text": original, "operation": "rot13"}))
        # Extract the result
        encoded = result1.split("Result: ")[1].strip()
        result2 = self.tool.use(json.dumps({"text": encoded, "operation": "rot13"}))
    ...
    def test_ascii_to_binary(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "Hi", "operation": "ascii_to_binary"})
        )
    ...
    def test_binary_to_ascii(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "01001000 01101001", "operation": "binary_to_ascii"})
        )
        assert "Hi" in result
    ...
    def test_jwt_decode_valid(self):
        ...
        result = self.tool.use(json.dumps({"text": token, "operation": "jwt_decode"}))
    ...
    def test_jwt_decode_invalid(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "not.a.valid.jwt", "operation": "jwt_decode"})
        )
    ...
    def test_unicode_normalize(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "ＨＥＬＬｏ", "operation": "unicode_normalize"})
        )
        assert "HELLo" in result
    ...
    def test_reverse_string(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "Hello World", "operation": "reverse_string"})
        )
...
class TestEncodingToolNewOperations:
    ...
    def test_base32_encode_empty_string(self):
        ...
        result = self.tool.use(json.dumps({"text": "", "operation": "base32_encode"}))
        assert "Result: " in result

    def test_base32_encode_result_format(self):
        """Test base32 encode output follows the expected format."""
        result = self.tool.use(
            json.dumps({"text": "test", "operation": "base32_encode"})
        )
    ...
    def test_base32_decode_simple(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "JBSWY3DP", "operation": "base32_decode"})
        )
        assert "Hello" in result
    ...
    def test_base32_decode_with_padding(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "JBUQ====", "operation": "base32_decode"})
        )
        assert "Hi" in result
    ...
    def test_base32_decode_binary_returns_hex(self):
        ...
        result = self.tool.use(
            json.dumps({"text": b32_encoded, "operation": "base32_decode"})
        )
        assert "Binary data" in result or "hex" in result.lower()
    ...
    def test_base32_roundtrip(self):
        ...
        result1 = self.tool.use(
            json.dumps({"text": original, "operation": "base32_encode"})
        )
        encoded = result1.split("Result: ")[1].strip()
        result2 = self.tool.use(
            json.dumps({"text": encoded, "operation": "base32_decode"})
        )
        assert original in result2
    ...
    def test_double_url_encode_angle_bracket(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "<", "operation": "double_url_encode"})
        )
    ...
    def test_double_url_encode_xss_payload(self):
        ...
        result = self.tool.use(
            json.dumps(
                {"text": "<script>alert(1)</script>", "operation": "double_url_encode"}
            )
        )
    ...
    def test_double_url_encode_already_safe_chars(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "abc123", "operation": "double_url_encode"})
        )
    ...
    def test_double_url_encode_result_format(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "test&value", "operation": "double_url_encode"})
        )
    ...
    def test_unicode_escape_non_ascii(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "\u00e9", "operation": "unicode_escape"})
        )
        assert "\\u00e9" in result
    ...
    def test_unicode_escape_keeps_alphanums(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "abc123", "operation": "unicode_escape"})
        )
        assert "abc123" in result
    ...
    def test_unicode_escape_special_chars(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "a<b>c", "operation": "unicode_escape"})
        )
    ...
    def test_unicode_escape_emoji(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "\u2603", "operation": "unicode_escape"})
        )
        assert "\\u2603" in result
    ...
    def test_unicode_unescape_non_ascii(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "caf\\u00e9", "operation": "unicode_unescape"})
        )
    ...
    def test_unicode_escape_unescape_roundtrip(self):
        ...
        original = "\u00e9\u00e8\u00ea"
        result1 = self.tool.use(
            json.dumps({"text": original, "operation": "unicode_escape"})
        )
        escaped = result1.split("Result: ")[1].strip()
        result2 = self.tool.use(
            json.dumps({"text": escaped, "operation": "unicode_unescape"})
        )
        assert original in result2
    ...
    def test_xor_basic(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "A", "operation": "xor", "key": "20"})
        )
        assert "a" in result

    def test_xor_no_key_returns_error(self):
        """Test XOR without key returns an error message."""
        result = self.tool.use(json.dumps({"text": "hello", "operation": "xor"}))
    ...
    def test_xor_repeating_key(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "AB", "operation": "xor", "key": "ff"})
        )
    ...
    def test_xor_binary_result_returns_hex(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "Hello", "operation": "xor", "key": "ff"})
        )
    ...
    def test_xor_with_multi_byte_key(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "AAAA", "operation": "xor", "key": "0102"})
        )
    ...
    def test_xor_roundtrip(self):
        ...
        key = "ab"
        result1 = self.tool.use(
            json.dumps({"text": original, "operation": "xor", "key": key})
        )
        ...
        result1 = self.tool.use(
            json.dumps({"text": "Hi", "operation": "xor", "key": "01"})
        )
        intermediate = result1.split("Result: ")[1].strip()
        result2 = self.tool.use(
            json.dumps({"text": intermediate, "operation": "xor", "key": "01"})
        )
    ...
    def test_octal_encode_simple(self):
        ...
        result = self.tool.use(json.dumps({"text": "A", "operation": "octal_encode"}))
        # ord('A') == 65 == 0o101 -> \101
    ...
    def test_octal_encode_hello(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "Hello", "operation": "octal_encode"})
        )
    ...
    def test_octal_encode_space_separated(self):
        ...
        result = self.tool.use(json.dumps({"text": "AB", "operation": "octal_encode"}))
    ...
    def test_octal_encode_result_format(self):
        ...
        result = self.tool.use(json.dumps({"text": "X", "operation": "octal_encode"}))
    ...
    def test_octal_decode_backslash_format(self):
        ...
        result = self.tool.use(
            json.dumps(
                {"text": "\\110\\145\\154\\154\\157", "operation": "octal_decode"}
            )
        )
        assert "Hello" in result
    ...
    def test_octal_decode_space_separated(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "110 145 154 154 157", "operation": "octal_decode"})
        )
        assert "Hello" in result
    ...
    def test_octal_encode_decode_roundtrip(self):
        ...
        original = "flag{octal}"
        result1 = self.tool.use(
            json.dumps({"text": original, "operation": "octal_encode"})
        )
        encoded = result1.split("Result: ")[1].strip()
        result2 = self.tool.use(
            json.dumps({"text": encoded, "operation": "octal_decode"})
        )
        assert original in result2
    ...
    def test_octal_decode_single_char(self):
        ...
        result = self.tool.use(
            json.dumps({"text": "\\101", "operation": "octal_decode"})
        )
        assert "A" in result
...
class TestHashIdentifierTool:
    ...
    def test_md5_hash(self):
        ...
        # MD5 hash of "hello"
        result = self.tool.use(json.dumps({"hash": "5d41402abc4b2a76b9719d911017c592"}))
    ...
    def test_sha1_hash(self):
        ...
        # SHA-1 hash of "hello"
        result = self.tool.use(
            json.dumps({"hash": "aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d"})
        )
    ...
    def test_sha512_hash(self):
        ...
        result = self.tool.use(json.dumps({"hash": sha512}))
    ...
    def test_bcrypt_hash(self):
        ...
        result = self.tool.use(json.dumps({"hash": bcrypt_hash}))
        assert "bcrypt" in result.lower()

    def test_unknown_hash(self):
        """Test handling of unknown hash format."""
        result = self.tool.use(
            json.dumps({"hash": "this_is_not_a_valid_hash_format_12345"})
        )
...
class TestEncodingToolCTFScenarios:
    ...
    def test_ctf_nested_encoding(self):
        ...
        # First decode base64
        result1 = self.tool.use(
            json.dumps({"text": b64_encoded, "operation": "base64_decode"})
        )
        ...
        # Then URL decode
        result2 = self.tool.use(
            json.dumps({"text": intermediate, "operation": "url_decode"})
        )
    ...
    def test_jwt_admin_claim(self):
        ...
        payload = (
            base64.urlsafe_b64encode(
                b'{"user":"guest","role":"admin","iat":1234567890}'
            )
            .decode()
            .rstrip("=")
        )
