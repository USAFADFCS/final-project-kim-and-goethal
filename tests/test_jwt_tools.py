...
class TestJwtModifyClaim:
    ...
    def test_modify_claim_missing_token(self):
        ...
        tool = JwtTool()
        result = tool.use(
            json.dumps({"operation": "modify_claim", "claims": {"admin": True}})
        )
        assert "Error" in result
        assert "token" in result.lower()
    ...
    def test_modify_claim_missing_claims(self):
        ...
        result = tool.use(json.dumps({"operation": "modify_claim", "token": token}))
        assert "Error" in result
    ...
    def test_modify_claim_shows_changes(self):
        ...
        result = tool.use(
            json.dumps(
                {"operation": "modify_claim", "token": token, "claims": {"admin": True}}
            )
        )
...
class TestJwtCrack:
    ...
    def test_crack_not_found(self):
        ...
        result = tool.use(
            json.dumps(
                {"operation": "crack", "token": token, "wordlist": ["wrong1", "wrong2"]}
            )
        )
...
class TestJwtForgeWithKey:
    ...
    def test_forge_with_key_missing_claims(self):
        ...
        tool = JwtTool()
        result = tool.use(json.dumps({"operation": "forge_with_key", "key": "secret"}))
        assert "Error" in result
        assert "claims" in result.lower()
    ...
    def test_forge_with_key_missing_key(self):
        ...
        tool = JwtTool()
        result = tool.use(
            json.dumps({"operation": "forge_with_key", "claims": {"admin": True}})
        )
        assert "Error" in result
...
class TestJwtAnalyze:
    ...
    def test_analyze_detects_privilege_claims(self):
        ...
        token = create_test_jwt(
            {"alg": "HS256"}, {"user": "test", "admin": False, "role": "user"}
        )
...
class TestJwtToolCTFScenarios:
    ...
    def test_admin_privilege_escalation(self):
        ...
        original = create_test_jwt(
            {"alg": "HS256"}, {"username": "user", "admin": False}
        )
        decode_result = tool.use(json.dumps({"operation": "decode", "token": original}))
        assert "admin" in decode_result
        assert "False" in decode_result or "false" in decode_result

        # Step 2: Try alg:none attack
        forge_result = tool.use(
            json.dumps({"operation": "forge_none", "token": original})
        )
        assert "none" in forge_result.lower()

        # Step 3: Modify claims
        modify_result = tool.use(
            json.dumps(
                {
                    "operation": "modify_claim",
                    "token": original,
    ...
    def test_weak_secret_exploitation(self):
        ...
        crack_result = tool.use(json.dumps({"operation": "crack", "token": weak_token}))
    ...
    def test_empty_secret_detection(self):
        ...
        token = create_test_jwt({"alg": "HS256"}, {"user": "test"}, "")  # Empty secret
...
class TestJwtToolEdgeCases:
    ...
    def test_malformed_base64(self):
        ...
        result = tool.use(json.dumps({"operation": "decode", "token": "!!!.@@@.###"}))
        assert "Error" in result or "Could not" in result
...
class TestJwtConfusionAttack:
    ...
    def test_confusion_missing_public_key_shows_locations(self):
        ...
        result = tool.use(
            json.dumps({"operation": "confusion_rs256_hs256", "token": token})
        )
...
class TestJwtKidInjection:
    ...
    def test_kid_inject_missing_token_and_claims(self):
        ...
        tool = JwtTool()
        result = tool.use(json.dumps({"operation": "kid_inject", "key": "secret"}))
        assert "Error" in result
        assert "token" in result.lower() or "claims" in result.lower()
    ...
    def test_kid_inject_generates_path_traversal_tokens(self):
        ...
        result = tool.use(
            json.dumps({"operation": "kid_inject", "token": token, "key": "secret"})
        )
    ...
    def test_kid_inject_generates_sqli_tokens(self):
        ...
        tool = JwtTool()
        token = create_test_jwt(
            {"alg": "HS256", "typ": "JWT", "kid": "key1"}, {"user": "test"}
        )
    ...
    def test_kid_inject_preserves_payload(self):
        ...
        token = create_test_jwt(
            {"alg": "HS256", "typ": "JWT", "kid": "key1"}, original_payload
        )
    ...
    def test_kid_inject_dev_null_signed_with_empty_key(self):
        ...
        result = tool.use(
            json.dumps({"operation": "kid_inject", "token": token, "key": "anything"})
        )
...
class TestJwtAnalyzeNewRecommendations:
    ...
    def test_analyze_recommends_confusion_for_rs256(self):
        ...
        assert (
            "algorithm confusion" in result.lower() or "Algorithm confusion" in result
        )
